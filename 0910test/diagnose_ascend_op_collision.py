...
Example:
    python3 0910test/diagnose_ascend_op_collision.py \
        --vllm-root /home/a00821909/vllm-ascend \
        --scenario all

By default, child processes use exactly two custom OPP vendors, in this order:
the vllm-ascend sparse vendor and the system ``vendors/customize`` package.  A
known-old system ``vendors/custom_transformer`` is removed from inherited
LD_LIBRARY_PATH entries.  Source the base CANN and ATB environments first.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Additional system operator vendor needed by custom.compressor; "
            f"may be repeated (default: {DEFAULT_SYSTEM_VENDOR})"
        ),
    )
    parser.add_argument(
        "--scenario",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "ascend-op-collision-results",
    ...
    parser.add_argument(
        ...
        default="only",
        help=(
            "use only requested vendors (default), prepend them to inherited "
            "ASCEND_CUSTOM_OPP_PATH, or preserve the inherited value"
        ),
    )
    parser.add_argument(
        "--ld-debug",
...
def sanitize_inherited_library_path(current: str | None) -> str | None:
    """Exclude the known-old system custom_transformer from controlled runs."""
    if not current:
        return current
    paths = []
    for item in current.split(":"):
        normalized = item.rstrip("/") + "/"
        if normalized.startswith("/usr/local/Ascend/") and (
            SYSTEM_TRANSFORMER_FRAGMENT in normalized
        ):
            continue
...
def child_environment(
    ...
    binding: Path,
    vendor: Path,
    system_vendors: list[Path],
    scenario: str,
) -> dict[str, str]:
    env = os.environ.copy()
    inherited_opp = env.get("ASCEND_CUSTOM_OPP_PATH")
    requested_opp = ":".join(str(path) for path in (vendor, *system_vendors))
    if args.opp_mode == "prepend":
        env["ASCEND_CUSTOM_OPP_PATH"] = prepend_unique(requested_opp, inherited_opp)
    elif args.opp_mode == "only":
        env["ASCEND_CUSTOM_OPP_PATH"] = requested_opp
    elif inherited_opp is None:
        env.pop("ASCEND_CUSTOM_OPP_PATH", None)

    inherited_library_path = sanitize_inherited_library_path(
        env.get("LD_LIBRARY_PATH")
    )
    library_path = prepend_unique(str(binding.parent), inherited_library_path)
    for path in reversed((vendor, *system_vendors)):
        library_path = prepend_unique(str(path / "op_api/lib"), library_path)
...
def child_command(
    args: argparse.Namespace, scenario: str, system_vendors: list[Path]
) -> list[str]:
    command = [
        sys.executable,
...
def elf_aclnn_symbols(path: Path) -> list[str]:
    readelf = shutil.which("readelf")
    if readelf is None:
        return ["<readelf unavailable>"]
    try:
    ...
    return sorted(
        {
            ...
            if any(symbol in line for line in completed.stdout.splitlines())
        }
    )


def static_elf_inventory(vendor: Path, system_vendors: list[Path]) -> dict[str, Any]:
    roots = [vendor, *system_vendors]
    ...
    for path in sorted(candidates, key=str):
        ...
        item["aclnn_symbols"] = elf_aclnn_symbols(path)
        files.append(item)
...
def run_parent(args: argparse.Namespace) -> int:
    ...
    requested_system_vendors = args.system_vendor or [DEFAULT_SYSTEM_VENDOR]
    system_vendors = [path.expanduser().resolve() for path in requested_system_vendors]
    missing_vendors = [path for path in system_vendors if not path.is_dir()]
    if missing_vendors:
    ...
    parent_environment_path = args.output_dir / "parent-environment.json"
    parent_environment_path.write_text(
        json.dumps(parent_environment, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )

    inventory = static_elf_inventory(vendor, system_vendors)
    ...
    for item in inventory["files"]:
        ...
        print(
            ...
            f"symbols={item.get('aclnn_symbols')}"
        )
    results: dict[str, int] = {}
    for scenario in scenarios:
        print(f"\n{'#' * 24} {scenario} {'#' * 24}", flush=True)
        env = child_environment(args, binding, vendor, system_vendors, scenario)
        console_path = args.output_dir / f"console-{scenario}.log"
        with console_path.open("w", encoding="utf-8") as console:
            process = subprocess.Popen(
                child_command(args, scenario, system_vendors),
                env=env,
