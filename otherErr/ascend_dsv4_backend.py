...
def _sparse_attn_ops(is_draft=False):
    ...
    """
    if is_npu_arch35():
        return _dspark_draft_arch35_sparse_attn_ops()
        # return (
...
def _npu_rope_tail(
    ...
    """NPU-safe interleaved RoPE: rotate the last rope_dim features of x in-place
    via the custom partial-rotary kernel.

    GPU code uses complex64 (view_as_complex + complex multiply), which NPU
    does not support (aclnnIndex rejects DT_COMPLEX64). The custom kernel
    ``inplace_partial_rotary_mul`` does the same interleave rotation with fp32
    accumulation internally, which is both faster and more precise than the
    previous pure-torch even/odd split.
    """
    rope = Dsv4NpuRoPE.for_freqs(freqs_cis)
    cos, sin = rope.get_cos_sin(
        positions.to(torch.int64), torch.float32, view_4d=True
    )
    qk_nope_dim = x.shape[-1] - rope_dim
    # apply_rotary_mul_inplace expects [T, n_heads, head_dim]; unsqueeze 2-D
    # inputs (latent, index_k) so the kernel sees a head axis.
    squeezed = False
    if x.ndim == 2:
        x = x.unsqueeze(-2)
        squeezed = True
    Dsv4NpuRoPE.apply_rotary_mul_inplace(x, None, cos, sin, qk_nope_dim=qk_nope_dim)
    if squeezed:
        x = x.squeeze(-2)
    return x
...
def _npu_rope_fq4(
    ...
    from NPU-supported ops: _npu_rope_tail (custom partial-rotary kernel) +
    fake_quant_fp4 (pure torch).
    """
...
class CompressorAscendBackendMixin:
    ...
    def _build_npu_compress_metadata(self, forward_batch: ForwardBatch) -> None:
        ...
        # Compute c1/c2 loc from full loc (NPU allocator only covers c4/c128).
        # Must run AFTER the None-overwrite block above. c{ratio}_loc =
        # full_loc // ratio for tokens that complete a group; the
        # _low_ratio_compress_torch group_mask filters the rest.
        raw_loc = forward_batch.out_cache_loc
        if raw_loc is not None and raw_loc.numel() > 0:
            pos = forward_batch.positions.to(torch.int64)
            for ratio in self._dsv4_compress_ratios:
                if ratio not in (1, 2):
                    continue
                existing = getattr(fm, f"c{ratio}_loc", None)
    ...
    def _build_npu_compress_metadata_prefill(self, forward_batch: ForwardBatch) -> None:
        ...
        ratio_lists: dict = {
            r: [] for r in self._dsv4_unique_compress_ratios if r in (1, 2, 4, 128)
        }
        for idx in range(bs):
        ...
        for ratio in (1, 2, 4, 128):
            if ratio not in ratio_lists:
                continue
            bundle_loc = (
                getattr(bundle, f"out_c{ratio}_loc", None)
    ...
    def _compute_compress_locs(
        ...
        *,
        ...
        for ratio in self._dsv4_unique_compress_ratios:
            ...
            if ratio == 4 or  ratio == 1 or ratio == 2:
...
class DeepseekV4AscendAttnBackend(
    ...
    def __init__(
        ...
        self._dsv4_has_c1 = 1 in self._dsv4_compress_ratios
        self._dsv4_has_c2 = 2 in self._dsv4_compress_ratios
    ...
    def _init_dsv4_graph_metadata(self, bs: int, forward_mode: ForwardMode) -> None:
        ...
        metadata.c1_page_table = self.graph_metadata["c1_page_table"][:bs, :]
        metadata.c2_page_table = self.graph_metadata["c2_page_table"][:bs, :]
        ...
        c1_pad = min(n_tok, n_tok // 1 + bs)
        c2_pad = min(n_tok, n_tok // 2 + bs)
        ...
        metadata.c1_loc = torch.zeros(c1_pad, dtype=torch.int64, device=device)
        metadata.c2_loc = torch.zeros(c2_pad, dtype=torch.int64, device=device)
        ...
        metadata.positions_cmp_padding_c1 = torch.zeros(
            c128_pad, dtype=torch.int64, device=device
        )
        metadata.positions_cmp_padding_c2 = torch.zeros(
            c128_pad, dtype=torch.int64, device=device
        )
        ...
        metadata.kernel_metadata = {
            ...
            "fulla_metadata": self.graph_metadata["kernel_metadata_fulla"],
            "c2a_metadata": self.graph_metadata["kernel_metadata_c2a"],
    ...
    def _kernel_metadata_from_parts(
        ...
        *,
        ...
        use_external_draft_ops = self._is_dspark_draft_worker and is_npu_arch35()
        ...
        if self._dsv4_has_c1:
            ...
            metadata_op, _ = _sparse_attn_ops()
            kernel_metadata["fulla_metadata"] = metadata_op(**fulla_kwargs)
        ...
        if self._dsv4_has_c2:
            c2a_overrides = {
                "cmp_ratio": 2,
                "has_cmp_kv": True,
                "cmp_topk": self._dsv4_index_topk,
            }
            c2a_kwargs = c1a_kwargs | c2a_overrides
            metadata_op, _ = _sparse_attn_ops()
            kernel_metadata["c2a_metadata"] = metadata_op(**c2a_kwargs)
    ...
    def forward(
        ...
        *,
        ...
        if compress_ratio not in (0, 1, 2, 4, 128):
            raise ValueError(
                f"V4 attention expects compress_ratio in (0, 1, 2, 4, 128); got {compress_ratio}"
            )
    ...
    def _forward_swa_native(
        ...
        for i in range(B):
            ...
            if q_len_i == 0:
                continue
            if kv_len_i == 0:
                outputs.append(q.new_zeros(q_len_i, H, D))
                continue
    ...
    def _forward_compressed_native(
        ...
        """Pure PyTorch compressed attention — no NPU sparse kernel."""
        fm = self.forward_metadata
        pool = self.token_to_kv_pool
        ori_kv = pool.get_swa_buffer(layer.layer_id)
        ...
        cu_seqlens = fm.actual_seq_lengths_q_pa
        kv_lens = fm.actual_seq_lengths_kv
        swa_block_table = fm.swa_page_table
        ...
        for i in range(B):
            ...
            q_i = q[q_start:q_end]
            # SWA KV
            swa_kv = self._gather_paged_kv(
                ori_kv, swa_block_table[i], kv_len_i, D
            )
            # Compressed KV
            cmp_len_i = kv_len_i // compress_ratio
            if cmp_len_i > 0 and cmp_kv is not None:
                c_kv = self._gather_paged_kv(
                    cmp_kv, cmp_block_table[i], cmp_len_i, D
                )
            else:
                c_kv = None
            # Build keys: [swa_kv; compressed_kv]
            all_kv = swa_kv
            if c_kv is not None:
            ...
            q_global = q_pos + (kv_len_i - q_len_i)
            # SWA mask for first kv_len_i keys
            swa_causal = k_pos[:kv_len_i][None, :] <= q_global[:, None]
            swa_win = k_pos[:kv_len_i][None, :] >= (
    ...
    def _forward_swa(
        ...
        if self._is_dspark_draft_worker:
            ...
            _, attn_op = _sparse_attn_ops(is_draft=self._is_dspark_draft_worker)
    ...
    def _forward_compressed(
        ...
        pool = self.token_to_kv_pool
        if compress_ratio == 1:
            metadata = fm.kernel_metadata.get(f"fulla_metadata")
        else:
    ...
    def _low_ratio_compress_torch(self, layer, x, req, pos, fm, forward_batch):
        ...
        if ratio != 1 and not bool(group_mask.any()):
            return

        if ratio != 1:
            out_loc = out_loc[group_mask]
    ...
    def _low_ratio_compress_decode(
        ...
        if ratio == 1:
            ...
            completes = (seq_lens % 2) == 0
            out_loc = torch.where(
                completes, out_loc_raw // 2, torch.full_like(out_loc_raw, -1)
            )
    ...
    def _low_ratio_write_group(self, layer, pooled, slots, group_pos) -> None:
        ...
        if layer.indexer is not None and layer.indexer.owns_k:
            ...
            if is_npu_arch35():
                ...
                kv_quant, kv_scale = torch_npu.npu_dynamic_quant(k)
    ...
    def _low_ratio_index_topk_torch_a3(
        self, layer, x, q_lora, req, pos
    ) -> None:
        pool = self.token_to_kv_pool
        ratio = layer.compress_ratio
        ...
        for b, r in enumerate(torch.unique_consecutive(req).tolist()):
            ...
            lc = int(lens.max().item()) if lens.numel() > 0 else 0
            if lc == 0:
                continue
            ...
            for start in range(0, tok.numel(), rows_per_chunk):
                ...
                if masks is not None:
                    ...
                    s = s.masked_fill(~consume[b][rows], float("-inf"))
